...
Перед публикацией 075: пересобрать bundle, пройти проверки, закоммитить только
новые используемые WebP (не `artifacts-cutouts`, не `design-concepts`), push и
проверить production URL `?release=075`.
