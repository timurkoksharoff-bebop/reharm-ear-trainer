...
function renderMission(model){
  ...
  routeElement.replaceChildren(...progress.map((part,index)=>{
    ...
    li.innerHTML=`<span class="route-node" aria-hidden="true"><i class="node-degree${part.degree?' found':''}"></i><i class="node-quality${part.quality?' found':''}"></i></span><span class="route-copy" aria-hidden="true"><small>${String(index+1).padStart(2,'0')}</small><b class="${label?'visible':''}">${label||'·'}</b></span>`;return li;
  }));
  ...
  const answerPart=(kind,value)=>{const result=mission.answerPart(kind,value);if(!result.ignored)pad.feedback(result.correct,{complete:result.positionComplete||result.complete}).catch(()=>{});};
