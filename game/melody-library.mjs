...
export function createMelodyLibrary({overlay,action,enter,back,audio,isActive,launch}){
  ...
  function open(){
    ...
    function render(){
      ...
      for(const melody of found.slice(page*size,(page+1)*size)){
        const b=document.createElement('button'),name=document.createElement('b'),meta=document.createElement('small');name.textContent=melody.name;meta.textContent=`${({jazz:'Джаз',rock:'Рок',classical:'Классика'})[melody.genre]} · открыть`;b.append(name,meta);b.onclick=()=>show(melody);el('melody-list').append(b);
      }
  ...
  function show(melody){
    ...
    el('melody-title').textContent=melody.name;el('melody-description').textContent=`${({jazz:'Джаз',rock:'Рок',classical:'Классика'})[melody.genre]} · ${Math.round(melody.events.reduce((sum,e)=>sum+e[1],0)*60/(melody.tempo||120))} секунд · ${melody.source}. ${melody.genre==='rock'?'Вокальная линия; окончания нот приблизительные. ':''}${melody.license}`;
