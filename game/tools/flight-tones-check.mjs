...
}
{
 ...
 assert.equal(cubes.length,5);assert.equal(cubes.filter(c=>c.rotating).length,1);
 assert.equal(cubes.filter(c=>c.label==='♭7').length,1);
 assert(cubes.every(c=>c.cube));assert(!cubes.find(c=>c.rotating).faces.includes('♭7'));
}
